...
$query = sprintf("SELECT id, email, code, name, timezone, nickname FROM teachers WHERE id = %u",
...
?>
<h1><?php echo htmlspecialchars($row ? 'Update' : 'Create'); ?> Teacher</h1>
<form action="teachers_control.php" method="post">
	...
	<table>
			...
			<td><select name="timezone" id="timezone">
				...
				foreach ($zones as $zone) :
					...
					<?php
						...
						?>
					<option value="<?php echo htmlspecialchars($zone); ?>"<?php if ($zone == $row['timezone']) { echo ' selected="selected"'; } ?>><?php echo htmlspecialchars($zone_name); ?></option>
					<?php
					endif;
		...
		<tr>
			...
			<td><input name="nickname" type="text" id="nickname" value="<?php echo htmlspecialchars($row['nickname']); ?>" maxlength="32"></td>
		</tr>
		<tr>
